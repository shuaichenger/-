...
const gradients: Record<string, string> = {
  '#8B5CF6': 'linear-gradient(137deg, #8B5CF6 0%, #A78BFA 45%, #6D28D9 100%)',
  '#EC4899': 'linear-gradient(137deg, #EC4899 0%, #F472B6 45%, #BE185D 100%)',
  '#F97316': 'linear-gradient(137deg, #F97316 0%, #FB923C 45%, #EA580C 100%)',
};
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 max-w-[1400px] mx-auto">
          {projects.map((proj, idx) => {
            const grad = gradients[proj.accent] || gradients['#8B5CF6'];

            return (
              <FadeIn key={proj.num} delay={idx * 0.1} y={40}>
                {/* Outer glow */}
                <div className="relative flex flex-col group mx-auto w-full max-w-[400px]">
                  <div
                    className="absolute w-full h-full opacity-50 rounded-[40px] pointer-events-none"
                    style={{ background: grad, filter: 'blur(50px)' }}
                  />

                  {/* Card with gradient border */}
                  <motion.div
                    className="relative rounded-[30px] sm:rounded-[40px] z-10 overflow-hidden cursor-pointer
                      transition-all duration-300 hover:-translate-y-1"
                    style={{
                      border: '6px solid transparent',
                      background: `linear-gradient(#1A1A1C, #1A1A1C) padding-box, ${grad} border-box`,
                    }}
                  ...
                  >
                    ...
                    <div className="p-5 sm:p-6 md:p-7">
                      <div className="flex items-center gap-3 mb-2">
                        <span
                          className="font-black leading-none text-3xl sm:text-4xl md:text-5xl"
                      ...
                      <h3 className="text-white font-medium text-xl sm:text-2xl md:text-3xl mt-1 tracking-tight">
                        {proj.name}
                      </h3>
                      <p className="text-gray-400 text-[14px] leading-[1.6] mt-2 line-clamp-2">
                        {proj.description}
                      </p>
